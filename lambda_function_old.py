...
config_path = os.path.join(current_dir, "bunq_api_context.conf")
    ...
    try:
        ...
        if operation == "payment":
            ...
            payment_id = PaymentApiObject.create(
                amount=AmountObject(
                    event.get("amount", "1.00"), event.get("currency", "EUR")
                ),
            ...
            request_id = RequestInquiryApiObject.create(
                AmountObject(
                    event.get("amount", "10.00"), event.get("currency", "EUR")
                ),
                PointerObject(
                    type_=event.get("pointer_type", "EMAIL"),
            ...
            payment_id = PaymentApiObject.create(
                amount=AmountObject(
                    event.get("amount", "5.00"), event.get("currency", "EUR")
                ),
                counterparty_alias=PointerObject(
                    type_="IBAN",
